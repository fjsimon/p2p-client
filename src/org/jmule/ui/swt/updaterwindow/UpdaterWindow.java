...
public class UpdaterWindow implements JMuleUIComponent {
	...
	public void initUIComponents() {
		...
		shell.setText(StringLocalizer.StringLocalizer("updaterwindow.title"));
		...
		label.setText(JMConstants.JMULE_NAME + " " + StringLocalizer.StringLocalizer("updaterwindow.label.jmule_version") + " : ");
		...
		label = new Label(window_content,SWT.NONE);
		label.setFont(skin.getDefaultFont());
		label.setText(StringLocalizer.StringLocalizer("updaterwindow.label.available_version") + " : ");
		label.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_END));
		...
		label.setFont(skin.getDefaultFont());
		label.setForeground(skin.getDefaultColor());
		label.setText(StringLocalizer.StringLocalizer("updaterwindow.label.last_update") + " : ");
		label.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_END));
		...
		download_link.setText(StringLocalizer.StringLocalizer("updaterwindow.label.download_link"));
		...
		download_link.addMouseListener(new MouseAdapter() {	
			public void mouseDoubleClick(MouseEvent arg0) {
				...
				if (!Utils.launchProgram(path)) 
					Utils.showWarningMessage(shell, StringLocalizer.StringLocalizer("updaterwindow.error_open_url.title")
							, Localizer.get("updaterwindow.error_open_url",path));
			}
		...
		changelog_group.setText(StringLocalizer.StringLocalizer("updaterwindow.group.changelog"));
		...
		button_startup_check.setText(StringLocalizer.StringLocalizer("updaterwindow.check.startup_check"));
		...
		button_ok.setText(StringLocalizer.StringLocalizer("updaterwindow.button.ok"));
		...
		new JMThread(new JMRunnable() {
			public void JMRun() {
				...
				try {
					...
					SWTThread.getDisplay().asyncExec(new JMRunnable() {
						public void JMRun() {
							Utils.showWarningMessage(shell, StringLocalizer.StringLocalizer("updaterwindow.connect_failed_title"), StringLocalizer.StringLocalizer("updaterwindow.connect_failed")+"\n"+updater.getErrorCode());
	...
	private void setWindowStatus(WindowStatus status) {
		if (status == WindowStatus.CONNECTING) {
			...
			available_version.setText(StringLocalizer.StringLocalizer("updaterwindow.label.connecting"));
		...
		if (status == WindowStatus.FAILED) {
			changelog_text.setText("");
			available_version.setText(StringLocalizer.StringLocalizer("updaterwindow.label.failed"));
