...
public class SearchTab extends AbstractTab{
	...
	private void search() {
		if ((searchType.getSelectionIndex()==0)&&(!_core.getServerManager().isConnected())) {
			Utils.showWarningMessage(getShell(), Localizer.get("mainwindow.searchtab.not_connected_to_server_title"), Localizer.get("mainwindow.searchtab.not_connected_to_server"));
			return;
		}
		if ((searchType.getSelectionIndex()==1)&&(!_core.getJKadManager().isConnected())) {
			Utils.showWarningMessage(getShell(), StringLocalizer.StringLocalizer("mainwindow.searchtab.not_connected_to_kad_title"), StringLocalizer.StringLocalizer("mainwindow.searchtab.not_connected_to_kad"));
			return;
		}		
