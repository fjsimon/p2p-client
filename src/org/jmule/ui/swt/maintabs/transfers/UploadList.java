...
public class UploadList extends JMTable<UploadSession> implements Refreshable,UploadManagerListener{
	...
	public UploadList(Composite composite,JMuleCore core) {
		...
		addColumn(SWT.LEFT, SWTConstants.UPLOAD_LIST_FILE_NAME_COLUMN_ID, Localizer.get("mainwindow.transferstab.uploads.column.filename"), "", width);
		
		width = swt_preferences.getColumnWidth(SWTConstants.UPLOAD_LIST_FILE_SIZE_COLUMN_ID);
		addColumn(SWT.RIGHT, SWTConstants.UPLOAD_LIST_FILE_SIZE_COLUMN_ID, Localizer.get("mainwindow.transferstab.uploads.column.filesize"), "", width);
		
		width = swt_preferences.getColumnWidth(SWTConstants.UPLOAD_LIST_UPLOAD_SPEED_COLUMN_ID);
		addColumn(SWT.RIGHT, SWTConstants.UPLOAD_LIST_UPLOAD_SPEED_COLUMN_ID, Localizer.get("mainwindow.transferstab.uploads.column.uploadspeed"), "", width);
		
		width = swt_preferences.getColumnWidth(SWTConstants.UPLOAD_LIST_PEERS_COLUMN_ID);
		addColumn(SWT.RIGHT, SWTConstants.UPLOAD_LIST_PEERS_COLUMN_ID, Localizer.get("mainwindow.transferstab.uploads.column.peers"), "", width);
		
		width = swt_preferences.getColumnWidth(SWTConstants.UPLOAD_LIST_ETA_COLUMN_ID); 
		addColumn(SWT.RIGHT, SWTConstants.UPLOAD_LIST_ETA_COLUMN_ID, Localizer.get("mainwindow.transferstab.uploads.column.eta"), StringLocalizer.StringLocalizer("mainwindow.transferstab.uploads.column.eta.desc"), width);
		
		width = swt_preferences.getColumnWidth(SWTConstants.UPLOAD_LIST_UPLOADED_COLUMN_ID);
		addColumn(SWT.RIGHT, SWTConstants.UPLOAD_LIST_UPLOADED_COLUMN_ID, Localizer.get("mainwindow.transferstab.uploads.column.uploaded"), "", width);
		...
		column_setup.setText(Localizer.get("mainwindow.transferstab.uploads.popupmenu.columnsetup"));
		...
		open_shared_file.setText(StringLocalizer.StringLocalizer("mainwindow.sharedtab.popupmenu.open"));
		...
		upload_details.setText(Localizer.get("mainwindow.transferstab.uploads.popupmenu.details"));
	...
	public void refresh() {
		...
		for(UploadSession session : upload_manager.getUploads()) {
			if (hasObject(session)) {
				...
				if (session.sharingCompleteFile()) {
					addRow(session);
					MainWindow.getLogger().fine(Localizer.get("mainwindow.logtab.message_upload_added",session.getSharingName()));
				}
	...
	public void uploadRemoved(final FileHash fileHash) {
		...
		SWTThread.getDisplay().asyncExec(new JMRunnable() {
			public void JMRun() {
				...
				if (session.sharingCompleteFile()) {
					MainWindow.getLogger().fine(Localizer.get("mainwindow.logtab.message_upload_removed",session.getSharingName()));
				}
		}});
	}
