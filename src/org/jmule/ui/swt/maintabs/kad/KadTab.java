 ...
 * Last changed by $Author: binary255 $ on $Date: 2010/07/28 13:22:44 $
 */
public class KadTab extends AbstractTab {
	...
	public KadTab(Composite shell, JMuleCore core) {		
		...
		routing_table_container.setText(StringLocalizer.StringLocalizer("mainwindow.kadtab.kad_nodes"));
		kad_tasks_container.setText(StringLocalizer.StringLocalizer("mainwindow.kadtab.kad_tasks"));
		...
		if (_jkad.isConnected())
			setKadStatus.setText(StringLocalizer.StringLocalizer("mainwindow.kadtab.disconnect"));
		if (_jkad.isConnecting())
			setKadStatus.setText(StringLocalizer.StringLocalizer("mainwindow.kadtab.stop_connecting"));
		if (_jkad.isDisconnected())
			setKadStatus.setText(StringLocalizer.StringLocalizer("mainwindow.kadtab.connect"));
		...
		contact_list.addColumn(SWT.LEFT, KAD_CLIENT_ID_COLUMN_ID, StringLocalizer.StringLocalizer("mainwindow.kadtab.contact_list.column.contact_id"), StringLocalizer.StringLocalizer("mainwindow.kadtab.contact_list.column.contact_id.desc"), swtPreferences.getColumnWidth(KAD_CLIENT_ID_COLUMN_ID));
		contact_list.addColumn(SWT.LEFT, KAD_CLIENT_DISTANCE_COLUMN_ID, StringLocalizer.StringLocalizer("mainwindow.kadtab.contact_list.column.contact_distance"), StringLocalizer.StringLocalizer("mainwindow.kadtab.contact_list.column.contact_distance.desc"), swtPreferences.getColumnWidth(KAD_CLIENT_DISTANCE_COLUMN_ID));
			...
			public void contactAdded(final KadContact contact) {
				Display.getDefault().asyncExec(new Runnable() {
					public void run() {
						...
						routing_table_container.setText(StringLocalizer.StringLocalizer("mainwindow.kadtab.kad_nodes_number",contact_list.getItemCount()+""));
					}
		...
		kad_task_list.addColumn(SWT.LEFT, KAD_TASK_TYPE_COLUMN_ID, StringLocalizer.StringLocalizer("mainwindow.kadtab.task_list.column.task_type"), StringLocalizer.StringLocalizer("mainwindow.kadtab.task_list.column.task_type.desc"), swtPreferences.getColumnWidth(KAD_TASK_TYPE_COLUMN_ID));
		kad_task_list.addColumn(SWT.LEFT, KAD_TASK_LOOKUP_HASH_COLUMN_ID, StringLocalizer.StringLocalizer("mainwindow.kadtab.task_list.column.task_hash"), StringLocalizer.StringLocalizer("mainwindow.kadtab.task_list.column.task_hash.desc"), swtPreferences.getColumnWidth(KAD_TASK_LOOKUP_HASH_COLUMN_ID));
		kad_task_list.addColumn(SWT.LEFT, KAD_TASK_LOOKUP_INFO_COLUMN_ID, StringLocalizer.StringLocalizer("mainwindow.kadtab.task_list.column.task_info"), StringLocalizer.StringLocalizer("mainwindow.kadtab.task_list.column.task_info.desc"), swtPreferences.getColumnWidth(KAD_TASK_LOOKUP_INFO_COLUMN_ID));
	...
	private void addPublishTask(PublishTask task) {
		...
		if (task instanceof PublishKeywordTask) {
			t.task_type = StringLocalizer.StringLocalizer("mainwindow.kadtab.lookup.type.keyword_publish");
			t.task_info = ((PublishKeywordTask) task).getKeyword();
		}
		
		if (task instanceof PublishSourceTask) {
			t.task_type = StringLocalizer.StringLocalizer("mainwindow.kadtab.lookup.type.source_publish");
			t.task_info = Utils.KadFileIDToFileName(task.getPublishID());
		}
		
		if (task instanceof PublishNoteTask) {
			t.task_type = StringLocalizer.StringLocalizer("mainwindow.kadtab.lookup.type.note_publish");
			t.task_info = Utils.KadFileIDToFileName(task.getPublishID());
		}
	...
	private void addSearchTask(SearchTask task) {
		...
		if (task instanceof KeywordSearchTask) {
			t.task_type = StringLocalizer.StringLocalizer("mainwindow.kadtab.lookup.type.keyword_search");
			t.task_info = ((KeywordSearchTask) task).getKeyword();
		}
		
		if (task instanceof SourceSearchTask) {
			t.task_type = StringLocalizer.StringLocalizer("mainwindow.kadtab.lookup.type.source_search");
			t.task_info = Utils.KadFileIDToFileName(task.getSearchID());
		}
		
		if (task instanceof NoteSearchTask) {
			t.task_type = StringLocalizer.StringLocalizer("mainwindow.kadtab.lookup.type.note_search");
			t.task_info = Utils.KadFileIDToFileName(task.getSearchID());
		}
