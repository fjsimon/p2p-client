 ...
 */
public class SearchTab extends MainTab {
	...
	public SearchTab(Composite ctab, JMuleCore core) {
		super(ctab,"Search");
		search_tab = this;
		GridLayout layout;
		_core = core;
				...
				SWTThread.getDisplay().asyncExec(new JMRunnable() {
					public void JMRun() {
						...
						if (tab != null) {
							...
							MainWindow.getLogger().fine(Localizer.get("mainwindow.logtab.message_search_result_arrived",
		...
		tab_content.setLayout(new GridLayout(1,true));
		
		Composite search_bar_composite = new Composite(tab_content, SWT.NONE);
		...
		label.setText(StringLocalizer.StringLocalizer("mainwindow.searchtab.label.search") + " : ");
		...
		search_button.setText(Localizer.get("mainwindow.searchtab.button.search"));
		...
		searchType.add(StringLocalizer.StringLocalizer("mainwindow.searchtab.search_type.server"));
		searchType.add(StringLocalizer.StringLocalizer("mainwindow.searchtab.search_type.kad"));
		searchType.add(StringLocalizer.StringLocalizer("mainwindow.searchtab.search_type.kad_server"));
		searchType.add(StringLocalizer.StringLocalizer("mainwindow.searchtab.search_type.global"));
		searchType.select(0);
		
		Label adv_search = new Label(basic_search_controls,SWT.NONE);
		adv_search.setText(StringLocalizer.StringLocalizer("mainwindow.searchtab.label.advanced"));
		...
		clear_advanced_options.setText(StringLocalizer.StringLocalizer("mainwindow.searchtab.label.clear"));
		...
		search_query_tab_list = new CTabFolder(tab_content, SWT.BORDER);
	...
	private void search() {
		if ((searchType.getSelectionIndex()==0)&&(!_core.getServerManager().isConnected())) {
			Utils.showWarningMessage(tab_content.getShell(), Localizer.get("mainwindow.searchtab.not_connected_to_server_title"), Localizer.get("mainwindow.searchtab.not_connected_to_server"));
			return;
		}
		if ((searchType.getSelectionIndex()==1)&&(!_core.getJKadManager().isConnected())) {
			Utils.showWarningMessage(tab_content.getShell(), StringLocalizer.StringLocalizer("mainwindow.searchtab.not_connected_to_kad_title"), StringLocalizer.StringLocalizer("mainwindow.searchtab.not_connected_to_kad"));
			return;
		}		
	...
	private void showAdvancedOptions() {
		...
		String text = "";
		if ((minFileSize != 0)||(maxFileSize!=0))
			text = " "+ StringLocalizer.StringLocalizer("mainwindow.searchtab.label.min_file_size")+" : "+FileFormatter.formatFileSize(minFileSize)+" "
			+ StringLocalizer.StringLocalizer("mainwindow.searchtab.label.max_file_size")+" : "+ FileFormatter.formatFileSize(maxFileSize)+" ";
		if (extension.length()!=0)
			text += StringLocalizer.StringLocalizer("mainwindow.searchtab.label.file_extension") + " : "+extension+" ";
		
		if (availableSources!=0)
			text += StringLocalizer.StringLocalizer("mainwindow.searchtab.label.file_availability") + " : " + availableSources+" ";
		
		if (completedSources!=0)
			text+= StringLocalizer.StringLocalizer("mainwindow.searchtab.label.file_sources") + " : "+completedSources + " ";
		if (fileType != FileType.ANY)
			text += StringLocalizer.StringLocalizer("mainwindow.searchtab.label.file_type") + " : " + fileType + " ";
		if (text.length()!=0) 
