...
public class SWTServerListWrapper {
	...
	private SWTServerListWrapper(ServerManager serverManager) {
		...
		server_manager.addServerListListener(new ServerManagerListener() {
			...
			public void serverAdded(final Server server) {
				SWTThread.getDisplay().asyncExec(new JMRunnable() {
					public void JMRun() {
						MainWindow.getLogger().fine(Localizer.get("mainwindow.logtab.message_server_added",server.getAddress()+":"+server.getPort()));
			...
			public void serverListCleared() {
				SWTThread.getDisplay().asyncExec(new JMRunnable() {
					public void JMRun() {
						MainWindow.getLogger().fine(Localizer.get("mainwindow.logtab.message_server_list_cleared"));
						server_list.clear();
						server_list_tab.setServerCount(server_manager.getServersCount());
				}});
			}
			...
			public void serverRemoved(final Server server) {
				SWTThread.getDisplay().asyncExec(new JMRunnable() {
					public void JMRun() {
						MainWindow.getLogger().fine(Localizer.get("mainwindow.logtab.message_server_removed",server.getAddress()+":"+server.getPort()));
	...
	private void setUIConnecting(final Server server) {
		...
		SWTThread.getDisplay().asyncExec(new JMRunnable() {
			public void JMRun() {
				MainWindow.getLogger().fine(StringLocalizer.StringLocalizer("mainwindow.logtab.message_connecting_to",server.getAddress()+":"+server.getPort()));
				if (!connect_button.isDisposed())
					connect_button.setConnecting();
				if (!status_bar.isDisposed())
					status_bar.setStatusConnecting();
