...
public class UploadPeerListTab extends CTabItem implements Refreshable{
	...
	public UploadPeerListTab(CTabFolder tabFolder, UploadSession uploadSession) {
		...
		setText(Localizer.get("uploadinfowindow.tab.peerlist.title"));
		...
		column_setup.setText(StringLocalizer.StringLocalizer("uploadinfowindow.tab.peerlist.menu.column_setup"));
	...
	private class UploadPeerList extends JMTable<Peer> {
		...
		public UploadPeerList(Composite composite) {
			...
			int width;
			addColumn(SWT.LEFT, SWTConstants.UPLOAD_PEER_LIST_NICKNAME_COLUMN_ID, StringLocalizer.StringLocalizer("uploadinfowindow.tab.peerlist.column.nickname"), "",	SWTPreferences.getInstance().getColumnWidth(SWTConstants.UPLOAD_PEER_LIST_NICKNAME_COLUMN_ID));
			
			width = swt_preferences.getColumnWidth(SWTConstants.DOWNLOAD_PEER_LIST_CC_COLUMN_ID);
			addColumn(SWT.CENTER, SWTConstants.UPLOAD_PEER_LIST_CC_COLUMN_ID, 		StringLocalizer.StringLocalizer("uploadinfowindow.tab.peerlist.column.country_code"), "", 		width );
			...
			addColumn(SWT.LEFT, SWTConstants.UPLOAD_PEER_LIST_FLAG_COLUMN_ID, 		StringLocalizer.StringLocalizer("uploadinfowindow.tab.peerlist.column.flag"), "", 		width );
			if (CountryLocator.getInstance().isServiceDown())
			...
			addColumn(SWT.LEFT, SWTConstants.UPLOAD_PEER_LIST_IP_COLUMN_ID, 			StringLocalizer.StringLocalizer("uploadinfowindow.tab.peerlist.column.address"), "", width);
			
			width = swt_preferences.getColumnWidth(SWTConstants.UPLOAD_PEER_LIST_UPLOAD_SPEED_COLUMN_ID);
			addColumn(SWT.RIGHT, SWTConstants.UPLOAD_PEER_LIST_UPLOAD_SPEED_COLUMN_ID, StringLocalizer.StringLocalizer("uploadinfowindow.tab.peerlist.column.up_speed"), "", width);
			
			width = swt_preferences.getColumnWidth(SWTConstants.UPLOAD_PEER_LIST_SOFTWARE_COLUMN_ID);
			addColumn(SWT.LEFT, SWTConstants.UPLOAD_PEER_LIST_SOFTWARE_COLUMN_ID, StringLocalizer.StringLocalizer("uploadinfowindow.tab.peerlist.column.software"), "", 	width);
			
			width = swt_preferences.getColumnWidth(SWTConstants.UPLOAD_PEER_LIST_STATUS_COLUMN_ID);
			addColumn(SWT.LEFT, SWTConstants.UPLOAD_PEER_LIST_STATUS_COLUMN_ID, StringLocalizer.StringLocalizer("uploadinfowindow.tab.peerlist.column.status"), "", width);
		...
		public void updateRow(Peer object) {
			...
			if (rank!=0) {
				...
				str = StringLocalizer.StringLocalizer("uploadinfowindow.tab.peerlist.column.status.uploading");
			}
