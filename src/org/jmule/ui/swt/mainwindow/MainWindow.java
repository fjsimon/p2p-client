	...
	public void initUIComponents() {
		...
		display.asyncExec(new JMRunnable() {
            public void JMRun() {
        		...
        		info_text.setText(StringLocalizer.StringLocalizer("mainwindow.no_tabs"));
            	...
            	new JMThread(new JMRunnable() {
            		public void JMRun() {
            			logger.fine(Localizer.get("mainwindow.logtab.message_jmule_started",  JMConstants.JMULE_FULL_NAME));
            			logger.fine(Localizer.get("mainwindow.logtab.message_servers_loaded", _core.getServerManager().getServersCount() + ""));
            			logger.fine(Localizer.get("mainwindow.logtab.message_shared_loaded", _core.getSharingManager().getFileCount() + ""));
            			logger.fine(Localizer.get("mainwindow.logtab.message_partial_loaded", _core.getDownloadManager().getDownloadCount() + ""));
            		}}).start();
							...
							display.syncExec(new JMRunnable() {
								...
								public void JMRun() {
									Utils.showErrorMessage(shell, 
											StringLocalizer.StringLocalizer("mainwindow.not_enough_space_dialog.title"),
            				...
            				try {
								...
								if (JMUpdater.getInstance().isNewVersionAvailable())
									SWTThread.getDisplay().asyncExec(new JMRunnable() {
				            			public void JMRun() {
				            				if (JMUpdater.getInstance().isNewVersionAvailable()) {
												if (Utils.showConfirmMessage(shell, StringLocalizer.StringLocalizer("mainwindow.new_version_available.title"), StringLocalizer.StringLocalizer("mainwindow.new_version_available"))) {
            	...
            	shell.addListener(SWT.Close, new Listener() {
					public void handleEvent(Event arg0) {
						boolean exit = SWTPreferences.getInstance().promptOnExit() ? 
								Utils.showConfirmMessage(shell, StringLocalizer.StringLocalizer("mainwindow.exit_prompt_title"), StringLocalizer.StringLocalizer("mainwindow.exit_prompt")) : true;
						arg0.doit = exit;
					}
            	});
